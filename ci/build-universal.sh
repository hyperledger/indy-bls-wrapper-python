...
TARGET_LIBS=
for target in $MACOS_UNIVERSAL_TARGETS; do
	echo "Building $LIB_NAME for toolchain '$BUILD_TOOLCHAIN', target '$target'.."
	$RUSTUP run $BUILD_TOOLCHAIN cargo build --release --features $FEATURES --target $target
